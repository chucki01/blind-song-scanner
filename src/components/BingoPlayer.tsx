...
export const BingoPlayer: React.FC<BingoPlayerProps> = ({ playlist, onBack }) => {
  ...
  const playNextSong = async () => {
    ...
    if (!previewUrl) {
      // Skip silently
      const newAvail = availableSongsRef.current;
    ...
    // Add to history immediately (but hidden in UI)
  ...
  const handleReveal = () => {
    // Already in reveal state, just showing the song
  };
